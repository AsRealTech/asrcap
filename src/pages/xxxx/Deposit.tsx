...
export default function Deposit() {
  ...
  const selectedMethod = selectedWallet
    ? depositMethods.find((method) => method.symbol === selectedWallet.currency)
    : null;
  const currentRate = selectedMethod?.rate ?? 0;
  const formattedCryptoAmount = selectedWallet && amount && currentRate
    ? (parseFloat(amount) / currentRate).toFixed(8)
    : "0.00000000";

  const handleWalletSelect = (wallet: any) => {
    const method = depositMethods.find((item) => item.symbol === wallet.currency);
        ...
        <div className="rounded-[32px] border border-border bg-card p-6 shadow-lg">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-bold text-foreground">Scroll and select your preferred payment method to deposit funds.</h1>
            </div>
            <button
              onClick={() => {
                setStep(1);
                      ...
                      <div key={wallet.id} className="rounded-3xl border border-border bg-white p-5 shadow-sm">
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                          <div className="flex items-center gap-4">
                            <div className={`flex h-14 w-14 items-center justify-center rounded-3xl ${method?.bg || "bg-slate-100"}`}>
                              {method ? (
                            ...
                            <div>
                              <p className="text-lg font-semibold text-foreground">{method?.name ?? wallet.currency} ({wallet.currency})</p>
                              <p className="text-sm text-muted-foreground">{method?.network ?? "Crypto Network"}</p>
                              <p className="text-sm text-muted-foreground mt-1">Rate: ${method?.rate?.toFixed(6) ?? "0.00"} per unit</p>
                            </div>
                          </div>
                          <button
                            type="button"
            ...
            <div className="mt-6 rounded-3xl border border-border bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
                <div className="rounded-3xl border border-border bg-slate-50 p-5">
                  <div className="flex items-center gap-4">
                    <div className={`flex h-14 w-14 items-center justify-center rounded-3xl ${selectedMethod?.bg ?? "bg-slate-100"}`}>
                      {selectedMethod ? (
                    ...
                    <div>
                      <p className="text-base font-semibold text-foreground">{selectedMethod?.name ?? selectedWallet.currency} ({selectedWallet.currency})</p>
                      <p className="text-sm text-muted-foreground">{selectedMethod?.network ?? "Crypto Network"}</p>
                      <p className="mt-2 text-xs text-muted-foreground">Rate: ${currentRate.toFixed(6)} per unit</p>
                    </div>
                  </div>
                </div>
          ...
          {step === 3 && selectedWallet && (
            <div className="mt-6 rounded-3xl border border-border bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 rounded-3xl border border-border bg-orange-50 px-5 py-4 text-orange-700">
                ...
                <p className="mt-2 text-base font-semibold">Please send {formattedCryptoAmount} {selectedWallet.currency} to the address below.</p>
              </div>
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
                <div className="rounded-3xl border border-border bg-white p-6 text-center shadow-sm">
                  <div className="mx-auto mb-4 h-40 w-40 rounded-3xl border border-border bg-slate-50" />
